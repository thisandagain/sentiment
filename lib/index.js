...
function negated(afinn, tokens, pos) {
    while (pos--) {
        var word = getWord(afinn, tokens[pos]);
        if (negators[word]) {
            return true;
...
module.exports = function (phrase, inject, callback) {
    ...
    while (len--) {
        var obj = getWord(afinn, tokens[len]);
        var item = afinn[obj];
        if (!afinn.hasOwnProperty(obj)) continue;

        // Check for negation
        if (len > 0 && negated(afinn, tokens, len)) {
            item = -item;
        }
